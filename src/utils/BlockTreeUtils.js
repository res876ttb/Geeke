...
export class BlockTree {
    ...
    const parseContentState = (curDepth) => {
      ...
      while (curBlock) {
        ...
        if (curDepth < blockDepth) {
          ...
          console.assert(
            curDepth + 1 === blockDepth,
            `The depth difference between curDepth and blockDepth must be only 1!`,
          )
  ...
  createParentMapFromTree(tree) {
    ...
    const createParentMap = (parentId, nodes) => {
      // nodes is an array of BlockLists
